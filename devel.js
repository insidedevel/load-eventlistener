...
(function() {
  ...
  function actualResizeHandler() {
    ...
    var in_xLoc, in_yLoc;
	    in_xLoc = window.outerWidth;
    in_yLoc = window.outerHeight;

    document.getElementById("userWidth").innerHTML = "Resized Width: " + in_xLoc;
    document.getElementById("userHeight").innerHTML = "Resized Height: " + in_yLoc;
  }
...
// see if window object is allowed
    if ($(window).length) {
        allowscriptJS = true;
	}
// see if document object is allowed
if ($(document).length) {} else {
    allowscriptJS = false;
}
...
function addWidthIn(el, speed) {
    var seconds = speed / 1000;
    el.style.transition = "max-width " + seconds + "s ease";
...
// document has been loaded event
window.addEventListener('load', function() {
    // retreive view height and width
    var in_xLoc, in_yLoc;
    in_xLoc = window.outerWidth;
    in_yLoc = window.outerHeight;
	document.getElementById("sitetitle").style.fontFamily = "aileronheavy";
    document.getElementById("userWidth").innerHTML = "Width: " + in_xLoc;
    document.getElementById("userHeight").innerHTML = "Height: " + in_yLoc;
	
	// Add fonts from style sheets depending on browser.
  ...
  if( supportsWoff2 ) {
    ...
    // Add Images to current display width
    ...
    for (var i = 0; i < allimages.length; i++) {
        if (allimages[i].getAttribute('data-src')) {
            ...
            // fix loaderContainer to be dynamic
            var divLoader = document.getElementById("loaderContainer");
            removeFadeOut(document.getElementById("loaderContainer"), 2000);
            addWidthIn(allimages[i], 1000);
        }
    }
}, false);
